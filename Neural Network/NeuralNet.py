...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        """YOUR CODE"""
        biasActs = inActs[:]
        biasActs.insert(0, 1.0)
        return (self.sigmoid(self.getWeightedSum(biasActs)))
    ...
    def sigmoidDeriv(self, value):
        ...
        return (exp(value) / ((exp(value) + 1) ** 2.0))
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        biasActs = inActs[:]
        biasActs.insert(0, 1.0)
        for i in range(len(biasActs)):
            sumSig += (self.weights[i] * biasActs[i])
        return self.sigmoidDeriv(sumSig)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(len(acts)):
            self.weights[i] += (alpha * delta * acts[i])
            totalModification += abs(alpha * delta * acts[i])
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            acts = [node.sigmoidActivation(acts) for node in layer]
            ret.append(acts)
        return ret
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for j in range(len(self.layers[layerNum + 1])): #
                        cumDelt += (deltas[0][j] * self.layers[layerNum + 1][j].weights[neuronNum + 1])  #
                    delta = gPrime * cumDelt
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    while (weightChangeThreshold < weightMod and iteration < maxItr):
        ret = nnet.backPropLearning(examplesTrain, alpha)
        trainError = ret[0]
        weightMod = ret[1]
        iteration += 1
    ...
    for example in examplesTest:
        ...
        satis = True
        i = 0
        while (i < (len(output[-1])) and satis):
            if (abs(round(output[-1][i]) - round(example[1][i])) > 0):
                testError += 1
                satis = False
            i += 1
        if (satis):
